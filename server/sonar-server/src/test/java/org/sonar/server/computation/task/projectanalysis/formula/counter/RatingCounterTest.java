...
public class RatingCounterTest {
  ...
  public void newly_created_value_is_unset_and_has_value_0() {
    verifyUnsetVariationValue(new RatingCounter());
  }

  @Test
  public void increment_sets_value_and_increments_value() {
    verifySetVariationValue(new RatingCounter().increment(B), B);
  }

  @Test
  public void increment_has_no_effect_if_arg_is_null() {
    verifyUnsetVariationValue(new RatingCounter().increment((RatingCounter) null));
  }

  @Test
  public void increment_has_no_effect_if_arg_is_unset() {
    verifyUnsetVariationValue(new RatingCounter().increment(new RatingCounter()));
  }

  @Test
  public void increment_increments_by_the_value_of_the_arg() {
    RatingCounter source = new RatingCounter().increment(B);
    RatingCounter target = new RatingCounter().increment(source);
  ...
  public void multiple_calls_to_increment_increments_by_the_value_of_the_arg() {
    RatingCounter target = new RatingCounter()
      .increment(new RatingCounter().increment(B))
      .increment(new RatingCounter().increment(D));
  ...
  public void multiples_calls_to_increment_increments_the_value() {
    RatingCounter variationValue = new RatingCounter()
      .increment(B)
      .increment(C);
  ...
  private static void verifyUnsetVariationValue(RatingCounter variationValue) {
    assertThat(variationValue.isSet()).isFalse();
    assertThat(variationValue.getValue()).isEqualTo(A);
  }

  private static void verifySetVariationValue(RatingCounter variationValue, Rating expected) {
